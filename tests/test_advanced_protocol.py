...
class AdvancedProtocolTests(unittest.TestCase):
    def test_motion_boundary_and_crimp_destination(self):
        self.assertEqual(protocol.motion_frame(1, "forward", "4294967295"), (CAN_ID_MOTION_CMD, b"\x01\xff\xff\xff\xff"))
    ...
    # bounded_int used to silently CLAMP an out-of-range value
    ...
    # becomes channel 0"). A garbled/out-of-range operator value must
    # never silently become a different, valid command.
    def test_out_of_range_values_are_rejected_not_clamped(self):
        with self.assertRaises(ValueError):
            protocol.motion_frame(1, "forward", "9999999999")  # exceeds the real 32-bit step count
        with self.assertRaises(ValueError):
            protocol.weld_frame(14, "9999")  # exceeds the real 2000ms weld pulse maximum
        with self.assertRaises(ValueError):
            protocol.paste_pulse_frame("-4", "999", "999")  # channel/duty/duration all out of range

    def test_weld_and_paste_bounds_match_protocol(self):
        self.assertEqual(protocol.weld_frame(14, "2000")[1], b"\x01\x07\xd0")
        self.assertEqual(protocol.paste_pulse_frame("0", "100", "255")[1], b"\x00\x64\xff")

    def test_bounded_int_rejects_a_bool_even_though_it_is_an_int_subclass(self):
        # isinstance(True, int) is True in Python (int(True) == 1) - a
        # literal True/False is never a meaningful count/duration/channel
        # value at this protocol boundary and must be rejected outright,
        # not silently accepted as 1/0.
        with self.assertRaises(ValueError):
            protocol.bounded_int(True, 0, 255)
        with self.assertRaises(ValueError):
            protocol.bounded_int(False, 0, 255)

    def test_clamp_for_display_still_clamps_deliberately_for_a_cosmetic_value(self):
        # The one real, deliberate exception - see global_status_frame's
        # own comment on why a status LED color is display-only, never a
        # movement/actuation value.
        self.assertEqual(protocol.clamp_for_display(999, 0, 255), 255)
        self.assertEqual(protocol.clamp_for_display(-5, 0, 255), 0)
        with self.assertRaises(ValueError):
            protocol.clamp_for_display(True, 0, 255)

    def test_motion_frame_rejects_an_unrecognized_direction(self):
        # this project's own exact reproduction: an unrecognized direction
        # must never silently fall through to encode as the OTHER,
        # opposite, equally-valid physical direction.
        with self.assertRaises(ValueError):
            protocol.motion_frame(1, "FORWARD", "1")  # wrong case
        with self.assertRaises(ValueError):
            protocol.motion_frame(1, "sideways", "1")  # not a real direction at all
        with self.assertRaises(ValueError):
            protocol.motion_frame(1, True, "1")  # a bool is never a valid direction string

    def test_drill_frame_rejects_an_unrecognized_direction(self):
        with self.assertRaises(ValueError):
            protocol.drill_frame("100", "Clockwise")  # wrong case
        with self.assertRaises(ValueError):
            protocol.drill_frame("100", "backwards")  # not a real direction at all
        # The real, pre-existing byte assignment must be unchanged for
        # both genuinely valid labels.
        self.assertEqual(protocol.drill_frame("100", "clockwise")[1], bytes([100, 0x00]))
        self.assertEqual(protocol.drill_frame("100", "counter-clockwise")[1], bytes([100, 0x01]))
