...
class PanelHelpersMixin:
    ...
    def _create_live_graph(self, parent, y_max, width=340, height=100, max_points=60, label="value"):
        ...
        everything before the last 60 points would defeat the entire
        point of exporting "for external analysis" in the first place."""
        canvas = tk.Canvas(parent, width=width, height=height, bg="#1a1a1a", highlightthickness=1,
                           highlightbackground="#444")
        canvas.pack(fill="x", padx=4, pady=(2, 6))
        values = []
        history: list[tuple[float, float]] = []  # (seconds_since_first_point, value) - unbounded, export-only
        ...
        def add_point(value):
            import time
            clamped = max(0, min(y_max, value))
            values.append(clamped)
            if len(values) > max_points:
                values.pop(0)
            t = time.monotonic()
            history.append((t - (history[0][0] if history else t), clamped))
        ...
        def export_csv():
            ...
            if not history:
                messagebox.showinfo(_("MSG_EXPORT_TITLE"), _("MSG_EXPORT_NO_DATA"))
                return
            path = filedialog.asksaveasfilename(
                defaultextension=".csv",
                filetypes=[("CSV", "*.csv")],
                initialfile=f"{label}.csv",
            )
            if not path:
                return
            try:
                with open(path, "w", newline="", encoding="utf-8") as f:
                    writer = csv.writer(f)
                    writer.writerow(["seconds", label])
                    writer.writerows(history)
                messagebox.showinfo(_("MSG_EXPORT_TITLE"), _("MSG_EXPORT_DONE", count=len(history), path=path))
            except OSError as e:
