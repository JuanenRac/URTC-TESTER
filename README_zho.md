...
- **MLX9064x 传感器变体**：**查询**显示 3 种 MLX9064x 系列成员（或
  完全没有）中当前配置的是哪一种（`0x1A7`——见 `CANBUS.TXT`）——仅当
...
测试 `docs/CANBUS.TXT` 中未（或尚未）记录的内容。除了 ID 范围和 DLC≤8
...
25 种配置文件的每一种都有自己的面板，直接基于 `docs/CANBUS.TXT` 构建：
...
| 电磁铁 | 激励/释放复选框 | 无 |
| 点焊头 | 脉冲持续时间 + 触发 | 无（只有当接触传感器先读取为高电平才会触发——见 `docs/CANBUS.TXT` 自身的 `0x1C0`） |
...
见 `docs/CANBUS.TXT`），因此是一次纯粹的一次性发送。
