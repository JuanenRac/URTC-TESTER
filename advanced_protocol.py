...
def bounded_int(value: str | int, minimum: int, maximum: int) -> int:
    """Parse an operator value STRICTLY for the protocol boundary; reject
    ...
    clamping for DISPLAY - see clamp_for_display() below - but the wire
    encoder itself must never turn invalid input into a different valid
    frame. isinstance(value, bool) is checked before int() even though
    bool is a subclass of int in Python (int(True) == 1) - a literal
    True/False is never a meaningful count/duration/channel value here.
    """
    ...
    if not (minimum <= parsed <= maximum):
        raise ValueError(f"value {parsed} is out of range [{minimum}, {maximum}]")
    return parsed


def clamp_for_display(value: str | int, minimum: int, maximum: int) -> int:
    """The clamping behavior bounded_int() used to have, explicitly kept
    for a UI widget's own on-screen bound (e.g. a live slider readout
    that should never show a value outside its travel) - never used to
    build a real CAN frame. See bounded_int()'s own docstring for why
    those two purposes must not share one function."""
    if isinstance(value, bool):
        raise ValueError("value must be an integer, not a boolean")
    try:
        parsed = int(value)
...
def _direction_byte(direction: str, label_for_0x01: str, label_for_0x00: str) -> int:
    ...
    equally-valid physical direction - the protocol boundary must reject
    that outright instead, even though the real UI only ever offers
    these two labels through an enum control (a UI constraint is not a
    protocol-boundary guarantee)."""
    if direction == label_for_0x01:
        return 0x01
    if direction == label_for_0x00:
        return 0x00
    raise ValueError(f"unknown direction {direction!r}: expected {label_for_0x01!r} or {label_for_0x00!r}")
...
def motion_frame(profile_id: int, direction: str, steps: str | int) -> tuple[int, bytes]:
    ...
    return command, bytes([_direction_byte(direction, "forward", "reverse")]) + struct.pack(">I", count)


def drill_frame(speed: str | int, direction: str) -> tuple[int, bytes]:
    # Original encoding preserved exactly: "counter-clockwise" -> 0x01, "clockwise" -> 0x00.
    return CAN_ID_DRILL_CMD, bytes([bounded_int(speed, 0, 255), _direction_byte(direction, "counter-clockwise", "clockwise")])
...
def global_status_frame(
    ...
    # value, so clamp_for_display() is the right, honest tool here.
    return CAN_ID_GLOBAL_STATUS, bytes([
        clamp_for_display(status_rgb[0], 0, 255), clamp_for_display(status_rgb[1], 0, 255), clamp_for_display(status_rgb[2], 0, 255),
        night_code,
        clamp_for_display(ring_rgb[0], 0, 255), clamp_for_display(ring_rgb[1], 0, 255), clamp_for_display(ring_rgb[2], 0, 255),
