...
**Versione:** 1.1.0 · **Autore:** JuanenRac (Electro Hobby 3D) &lt;electrohobby3d@gmail.com&gt;
...
`1.1.9` → `1.2.0`). Eseguire dal codice sorgente (`python urtc_tester.py`)
...
- **[HYDRA-UMC-SERVER](https://github.com/JuanenRac/HYDRA-UMC-SERVER)** — il backend headless (Node/Express/WebSocket) che in precedenza era integrato nel processo stesso di HYDRA-UMC-STUDIO. Gestisce l'API REST/WS di controllo robot, la persistenza di settings.json, l'autenticazione JWT e la scoperta mDNS. HYDRA-UMC-STUDIO è ora un client frontend statico puro che comunica con esso in rete.
